...
public class AdminService {
    ...
    public List<Admin> findAll(Integer hotelId) {
    ...
    private EntityWrapper getEntityWrapper(Admin admin){
        ...
        if(null != admin){
            ...
            if(!StringUtils.isEmpty(admin.getHotelId())){
                entityWrapper.eq("hotel_id",String.valueOf(admin.getHotelId()));
            }
        }
    ...
    public Result login(LoginQuery query) {
        ...
        payloadMap.put("hotelId",list.get(0).getHotelId().toString());
        String token = JWTUtils.getToken(payloadMap);
        return new Result(true, StatusCode.OK,"登录成功",token);
    }

    public Result register(Admin admin) {
        List<Admin> list = adminMapper.selectList(new EntityWrapper().eq("username", admin.getUsername()));
